...
def calc_ind_viol(preds, Xp):
    ...
    n = Xp.shape[0]
    gs = Xp.sum(axis=0) # shape 7 for adult
    pmean = preds.mean()
    gmeans = (preds @ Xp)/gs 
    gviols = gmeans - pmean
...
def calc_eo_viol(preds, Xp, y):
    ...
    # calculate equal opportunity fairness violation. uses a weird continuous version though
    # using predictions preds on protected att mat Xp
    n_y = max(1, np.sum(y)) # number of positive labels aka 1's
    n_g_ys = y @ Xp # [7, ]. number of 1's each group has
    y_mean = (preds @ y)/n_y # preds@y: scalar. y_mean = scalar, average probability per positive label
    g_y_means = ((preds * y) @ Xp)/np.maximum(1, n_g_ys) # preds*y = [n, ]. numerator = [7, ]. g_y_means = [7, ]. above but per group. os average prob per pos label by group.
    viols = (g_y_means - y_mean) * (n_g_ys > 0) # [7, ] difference between each group's average prob per pos, minus the total
    abs_viols = np.abs(viols)
    return max(np.max(abs_viols), np.max(abs_viols * n_g_ys / (n_y - n_g_ys)))  # second variable here:  frankly I am confused
