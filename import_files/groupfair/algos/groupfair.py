...
class Plugin(GroupFair):
    ...
    def min_oracle(self, lambda_, X, Xp, y):
        ...
        def min_pred(input_X, input_Xp):
            ...
            return final_costs[:, 0] # to return probability
        return min_pred
...
class WERM(GroupFair):
    ...
    def min_oracle(self, lambda_, X, Xp, y):
        ...
        if self.lr_type=='logistic':
            ...
            return lambda X, Xp: lr.predict_proba(X)[:, 1] # jelly: to get proba
            #return lambda X, Xp: [np.argmax(lr.predict_proba(X), axis=1), lr.predict_proba(X)[:, 1]]
